...
class COCODataset(JointsDataset):
    ...
    def __init__(self, cfg, root, image_set, is_train, transform=None):
        ...
        logger.info("=> num_images: %d", self.num_images)
        ...
        logger.info("=> load %d samples", len(self.db))
    ...
    def _load_coco_person_detection_results(self):
        ...
        logger.info("=> Total boxes: %d", len(all_boxes))

        kpt_db = []
        num_boxes = 0
        for _, det_res in enumerate(all_boxes):
            ...
            joints_3d = np.zeros((self.num_joints, 3), dtype=float)
            joints_3d_vis = np.ones((self.num_joints, 3), dtype=float)
            kpt_db.append(
                {
                    "image": img_name,
                    "center": center,
        ...
        logger.info(
            "=> Total boxes after fliter low score@%s: %s", self.image_thre, num_boxes
        )
        return kpt_db
    ...
    def evaluate(self, cfg, preds, output_dir, all_boxes, img_path, *args, **kwargs):
        ...
        #     img_kpts = kpts[img]
        for img, img_kpts in kpts.items():
            for n_p in img_kpts:
                box_score = n_p["score"]
                kpt_score = 0
                valid_num = 0
