...
class ColorStyle:
    def __init__(self, color, link_pairs, point_color):
        ...
        for i in range(len(color_ids)):
            self.color_ids.append(tuple(np.array(color_ids[i]) / 255.0))
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="tmp/dataset/COCO/val2017/",
    )
    parser.add_argument(
        "--gt-anno",
        help="Path of COCO val annotation",
        type=str,
        default="tmp/dataset/COCO/person_keypoints_val2017.json",
    )
    parser.add_argument(
        "--save-path",
...
def plot_COCO_image(
    preds, img_path, save_path, link_pairs, ring_color, color_ids, save=True
):
    # Read Images
...
def main(args):
    ...
    if cfg.TEST.MODEL_FILE:
        ...
        for k in model_state_dict:
            if (
                k in model_state_dict
                and model_state_dict[k].size() == model_state_dict[k].size()
            ):
    ...
    files_loc = "tmp/dataset/NSL/9303/195KB_1"
    ...
    for idx in range(len(images)):
        ...
        preds, maxvals = get_final_preds_no_transform(
            cfg, outputs.detach().cpu().numpy()
        )
        ...
        plot_COCO_image(
            4 * preds,
            img_path,
            "samples/NSL/9303/195KB_1/" + images[idx],
            colorstyle.link_pairs,
            colorstyle.ring_color,
            colorstyle.color_ids,
            save=True,
        )
