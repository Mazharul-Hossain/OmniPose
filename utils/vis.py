...
def save_images(
    batch_image, batch_joints, batch_joints_vis, output_dir, meta, iteration
):
    """
    ...
    gt_file = "tmp/dataset/MPII/annot/valid.json"
    ...
    for i in range(len(color_ids)):
        limb_colors.append(tuple(np.array(color_ids[i]) / 255.0))

    joint_colors = []
    for i in range(len(point_colors)):
        joint_colors.append(tuple(np.array(point_colors[i]) / 255.0))
    ...
    for i in range(batch_joints.shape[0]):
        ...
        if Counter(images_done)[file_name[i]] > 0:
            filename = (
                "output/mpii/omnipose/OmniPose_HRw48_v2/val_images/"
                + file_name[i][-13:]
            )
            print(filename)
        else:
        ...
        for j in range(gt_joints.shape[1]):
            if gt[i_gt]["joints_vis"][j] == 1:
                circle = mpatches.Circle(
                    (int(gt[i_gt]["joints"][j][0]), int(gt[i_gt]["joints"][j][1])),
                    radius=int(h / 100),
                    ec="black",
                    fc=joint_colors[j],
                    alpha=1,
        ...
        for k, link_pair in enumerate(link_pairs):
            if (
                gt[i_gt]["joints_vis"][link_pair[0]] == 1
                and gt[i_gt]["joints_vis"][link_pair[1]] == 1
            ):
                line = mlines.Line2D(
                    np.array(
                        [
                            gt[i_gt]["joints"][link_pair[0]][0],
                            gt[i_gt]["joints"][link_pair[1]][0],
                        ]
                    ),
                    np.array(
                        [
                            gt[i_gt]["joints"][link_pair[0]][1],
                    ...
                    lw=int(h / 100),
                    alpha=1,
                    color=limb_colors[Counter(images_done)[file_name[i]]],
                )
                line.set_zorder(0)
                ax.add_line(line)
        ...
        plt.margins(0, 0)
        plt.savefig(
            output_dir + "val_images/" + gt[i_gt]["image"][:-3] + "png",
            format="png",
            bbox_inckes="tight",
            dpi=100,
        )
