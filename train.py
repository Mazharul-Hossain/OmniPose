...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="experiments/mpii/hrnet/w48_256x256_adam_lr1e-3.yaml",
        type=str,
    )
    parser.add_argument(
        "--opts",
    ...
    parser.add_argument("--modelDir", help="model directory", type=str, default="tmp/output")
    parser.add_argument("--logDir", help="log directory", type=str, default="tmp/log")
...
def main(args):
    ...
    command_line = "copy models\\" + cfg.MODEL.NAME + ".py " + final_output_dir + "\\"
    ...
    if cfg.AUTO_RESUME and os.path.exists(checkpoint_file):
        ...
        print(
            f"Loading checkpoint with accuracy of {checkpoint['perf']} at epoch {checkpoint['epoch']}"
        )
        ...
        for k in model_state_dict:
            if (
                ...
                and model_state_dict[k].size() == checkpoint["state_dict"][k].size()
            ):
        ...
        logger.info(
            "=> loaded checkpoint %s (epoch %d)", checkpoint_file, checkpoint["epoch"]
        )
    ...
    for epoch in range(begin_epoch, cfg.TRAIN.END_EPOCH):
        ...
        if perf_indicator >= best_perf:
            ...
            logger.info("=> saving checkpoint to %s", final_output_dir)
            save_checkpoint(
                {
                    "epoch": epoch + 1,
    ...
    logger.info("=> saving final model state to %s", final_model_state_file)
